...
func TestTaskDeleteStaleVirtualMachineInstanceMigrations(t *testing.T) {
	tests := []struct {
		name          string
		client        compat.Client
		task          *Task
		expectedError bool
		expectedMsg   string
		expectedVMIMs []*virtv1.VirtualMachineInstanceMigration
	}{
		{
			name:   "No pvcs in either namespace, but persistent volume claims in DVM, should error on missing PVCs",
			client: getFakeClientWithObjs(),
			task: &Task{
				Owner: &v1alpha1.DirectVolumeMigration{
					Spec: v1alpha1.DirectVolumeMigrationSpec{
						PersistentVolumeClaims: []v1alpha1.PVCToMigrate{
							{
								ObjectReference: &corev1.ObjectReference{
									Name:      sourcePVC,
									Namespace: sourceNs,
								},
								TargetName:      targetPVC,
			...
			expectedError: true,
			expectedMsg:   "persistentvolumeclaims \"source-pvc\" not found",
		},
		{
			name:   "PVCs in different namespaces, and persistent volume claims in DVM, should error on mismatched namespaces",
			client: getFakeClientWithObjs(createPvc(sourcePVC, sourceNs), createPvc(targetPVC, targetNs)),
			task: &Task{
				Owner: &v1alpha1.DirectVolumeMigration{
					Spec: v1alpha1.DirectVolumeMigrationSpec{
						PersistentVolumeClaims: []v1alpha1.PVCToMigrate{
		...
		},
		{
			...
			client: getFakeClientWithObjs(createPvc(sourcePVC, sourceNs), createPvc(targetPVC, sourceNs)),
			task: &Task{
				Owner: &v1alpha1.DirectVolumeMigration{
					Spec: v1alpha1.DirectVolumeMigrationSpec{
						PersistentVolumeClaims: []v1alpha1.PVCToMigrate{
		...
		{
			name: "PVCs in same namespace, and persistent volume claims in DVM, VMs and VMIMs created before the DVM, should not return error, should delete VMIM",
			client: getFakeClientWithObjs(
				createPvc(sourcePVC, sourceNs),
				createPvc(targetPVC, sourceNs),
				createVirtualMachine("vm", sourceNs),
				createCompletedVirtualMachineMigration("vmim", sourceNs, "vm"),
			),
			task: &Task{
				Owner: &v1alpha1.DirectVolumeMigration{
		...
		{
			name: "PVCs in same namespace, and persistent volume claims in DVM, VMs and VMIMs created after the DVM, should not return error, should NOT delete VMIM",
			client: getFakeClientWithObjs(
				createPvc(sourcePVC, sourceNs),
				createPvc(targetPVC, sourceNs),
				createVirtualMachine("vm", sourceNs),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.task.sourceClient = tt.client
			tt.task.destinationClient = tt.client
			err := tt.task.deleteStaleVirtualMachineInstanceMigrations()
			if tt.expectedError {
				if err == nil {
					t.Errorf("expected error but got nil")
					t.FailNow()
			...
			remainingVMIMs := &virtv1.VirtualMachineInstanceMigrationList{}
			err = tt.client.List(context.Background(), remainingVMIMs, k8sclient.InNamespace(sourceNs), &k8sclient.ListOptions{})
			if err != nil {
				t.Errorf("error listing VMIMs: %v", err)
				t.FailNow()
			}
			if len(remainingVMIMs.Items) != len(tt.expectedVMIMs) {
				t.Errorf("expected %d VMIMs, got %d", len(tt.expectedVMIMs), len(remainingVMIMs.Items))
				t.FailNow()
			}
			for _, remainingVMIM := range remainingVMIMs.Items {
				found := false
				for _, expectedVMIM := range tt.expectedVMIMs {
					if remainingVMIM.Name == expectedVMIM.Name {
						found = true
						break
					}
				}
				if !found {
					t.Errorf("unexpected VMIM %s", remainingVMIM.Name)
					t.FailNow()
				}
			}
		})
