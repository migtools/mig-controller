...
// A registry health checker which routinely checks whether registries are healthy or not for migplan's failed migrations
type registryHealthFailed struct {
...
func (r *registryHealthFailed) Start(
	handler handler.EventHandler,
	queue workqueue.RateLimitingInterface,
...
func (r *registryHealthFailed) enqueue(plan v1alpha1.MigPlan) {
...
func (r *registryHealthFailed) run() {
	//List all the migplans that have migrations in failed state using the hostClient
	...
	for {
		...
		planList, err := v1alpha1.ListFailedPlans(r.hostClient)
		if err != nil {
			log.Trace(err)
			return
		}
